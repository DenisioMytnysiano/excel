...
public class MaxOperatorTests
{
    [Fact]
    public void Evaluate_Returns_MaximumOfValues_When_EvaluatedValuesAreDoubles()
    {
        ...
        var averageNode = new MaxOperatorNode(childNode);
        var result = averageNode.Evaluate(evaluationContext);
        
        Assert.True(result is DoubleResult);
        Assert.Equal(15, ((DoubleResult)result).Value);
    }
