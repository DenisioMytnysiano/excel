...
{
    [Fact]
    public void Evaluate_Returns_AverageOfValues_When_EvaluatedValuesAreDoubles()
    {
        ...
        var averageNode = new AverageOperatorNode(childNode);
        var result = averageNode.Evaluate(evaluationContext);
        
        Assert.True(result is DoubleResult);
        Assert.Equal(12.5, ((DoubleResult)result).Value);
    }
