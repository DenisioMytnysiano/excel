...
public class MinOperatorNode : UnaryOperatorNode
{
    public MinOperatorNode(Node node) : base(node)
    {
    }
    ...
    public override CellResult Evaluate(IEvaluationContext context)
    {
        ...
        var sum = rangeResult.Results.Cast<DoubleResult>().Select(x => x.Value).Min();
